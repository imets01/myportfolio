  ...
  return (
    <div className="w-full bg-background font-sans" ref={containerRef}>
      <div className="max-w-7xl mx-auto pt-20">
        <h2 className="text-2xl md:text-3xl font-semibold mb-4 text-primary max-w-4xl">
          {data.title}
        </h2>
      </div>
        ...
        {data.data.map((item, index) => (
          <div key={index} className="flex justify-start pt-32 md:gap-10">
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-8/12">
              ...
              <h3 className="hidden md:block text-xl md:text-3xl md:pl-20  font-bold text-muted-foreground">
                {item.year}
              </h3>
            </div>
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl md:text-xl mb-4 text-left font-bold text-muted-foreground">
                {item.year}
              </h3>
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  ...
                  <h5 className="text-xl md:text-2xl font-medium text-muted-foreground">
                    {item.institution || item.company}
                  </h5>
                </div>
              </div>
              <ul className="text-primary text-lg md:text-sm font-normal mt-2 list-disc pl-20">
